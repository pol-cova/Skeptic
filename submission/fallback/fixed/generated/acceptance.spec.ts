...
test('[1] An invalid email address shows a validation message and does not create an invitation.', async ({ page }) => {
  await page.request.post('http://127.0.0.1:3101/api/reset');
  ...
  await expect(page.getByTestId('invite-validation-error')).toContainText('valid email');
...
test('[2] A valid email address creates an invitation and displays it in the Pending invitations list.', async ({ page }) => {
  await page.request.post('http://127.0.0.1:3101/api/reset');
  await page.goto('http://127.0.0.1:3101/login');
  await page.getByTestId('login-username').fill('demo');
  await page.getByTestId('login-password').fill('skeptic-demo');
  await page.getByTestId('login-submit').click();
  await page.getByTestId('invite-email').waitFor({ timeout: 10000 });
  await page.getByTestId('invite-email').fill('verify-verify-1785117802371@example.com');
  await page.getByTestId('invite-submit').click();
  ...
  await page.goto('http://127.0.0.1:3101/team');
  await page.getByTestId('pending-invitations').waitFor({ timeout: 5000 });
  await expect(page.getByTestId('pending-invitation-row')).toHaveCount(1);
});

test('[3] Inviting the same email twice shows a duplicate-invitation error and does not create a second row.', async ({ page }) => {
  await page.request.post('http://127.0.0.1:3101/api/reset');
  await page.goto('http://127.0.0.1:3101/login');
  await page.getByTestId('login-username').fill('demo');
  await page.getByTestId('login-password').fill('skeptic-demo');
